workspace {
            ...
            nolus = softwareSystem "Nolus" {
                validatornode = group "Validator/Sentry Node" {
                    ...
                    contracts = container "Smart Contracts" {
                        ...
                        borrower = component "Borrower" "Provide quotes, open new Lease"
                        ...
                        borrower -> lease "open a new lease with % interest margin"
                        borrower -> lease "transfer the downpayment"
                        
                        time_oracle -> lease "time alarms"
                        price_oracle -> lease "price alarms"
                        lease -> stable_lpp "request loan UST"
                        lease -> stable_lpp "repay loan UST"
                        lease -> swap "exchange downpayment->crypto"
                        lease -> swap "exchange LPP loan UST->crypto"
                        lease -> swap "exchange repayment->UST"
    ...
    views {
        ...
        dynamic contracts "case0" "Lease successful close" {
            ...
            user -> borrower "quote lease(C: currency, D: down-payment)"
            borrower -> stable_lpp "quote % interest rate"
            borrower -> price_oracle "read C/UST, D/UST prices"
            borrower -> user "quote(A: amount in C, B: borrowed amount in UST, I: interest rate)"

            user -> borrower "open lease(C: currency) & transfer(D: down-payment)"
            borrower -> lease "instantiate(C, D, Ii: % interest margin)"
            borrower -> lease "transfer(D)"
            lease -> swap "exchange D->C"
            lease -> price_oracle "read C/UST, D/UST prices"
            lease -> stable_lpp "request (A-D) loan UST"
            lease -> swap "exchange UST->C"
            borrower -> user "open lease done"

            user -> lease "repay() & transfer(R: re-payment)"
            lease -> swap "exchange R->UST"
            lease -> stable_lpp "repay loan+LPP interest UST"
        ...
        dynamic contracts "case1" "Lease liquidation" {
            ...
            lease -> swap "exchange some C->UST"
