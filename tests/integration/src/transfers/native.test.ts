...
import {getUser1Client, getUser1Wallet, getUser2Wallet, getValidatorClient, getValidatorWallet} from "../util/clients";
...
describe("Native transfers", () => {
    ...
    test("Users can transfer tokens", async () => {
        ...
        let broadcastTxResponse1: DeliverTxResponse = await validatorClient.sendTokens(validatorAccount.address, user1Account.address, [transfer1], fee, "Testing send transaction");
        ...
        let broadcastTxResponse2: DeliverTxResponse = await user1Client.sendTokens(user1Account.address, user2Account.address, [transfer2], fee, "Testing send transaction");
